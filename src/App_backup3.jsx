  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800' : 'bg-gradient-to-br from-gray-50 via-white to-gray-100'} transition-all duration-500`}>
      <div className="flex h-screen">
        {/* Modern Sidebar */}
        <div className={`${sidebarOpen ? 'w-80' : 'w-16'} transition-all duration-500 ease-in-out ${darkMode ? 'bg-slate-900/95 backdrop-blur-xl border-slate-700/50' : 'bg-white/95 backdrop-blur-xl border-gray-200/50'} border-r flex flex-col shadow-2xl relative`}>
          {/* Header */}
          <div className="p-6 border-b border-slate-700/30">
            <div className="flex items-center justify-between">
              <div className={`${sidebarOpen ? 'block' : 'hidden'} transition-all duration-300`}>
                <h1 className="font-bold text-2xl bg-gradient-to-r from-emerald-400 via-teal-500 to-cyan-600 bg-clip-text text-transparent">
                  ASB Data Viewer
                </h1>
          ...
          {sidebarOpen && (
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              {/* File Upload Section */}
              <div className={`${darkMode ? 'bg-gradient-to-br from-slate-800/50 to-slate-700/30' : 'bg-gradient-to-br from-gray-50 to-white'} backdrop-blur-sm p-6 rounded-2xl border ${darkMode ? 'border-slate-700/50' : 'border-gray-200/50'} shadow-lg hover:shadow-xl transition-all duration-300`}>
                <div className="flex items-center mb-4">
                  ...
                  <h3 className={`font-semibold text-lg ml-3 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                    Data Upload
                  </h3>
                </div>
                <div
                  className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 ${isDragging
                      ? 'border-emerald-400 bg-emerald-50/50 dark:bg-emerald-900/20 scale-105'
                      : `${darkMode ? 'border-slate-600 hover:border-slate-500 bg-slate-800/30' : 'border-gray-300 hover:border-gray-400 bg-gray-50/50'}`
                    } hover:shadow-inner`}
                ...
                <div className={`${darkMode ? 'bg-gradient-to-br from-slate-800/50 to-slate-700/30' : 'bg-gradient-to-br from-gray-50 to-white'} backdrop-blur-sm p-6 rounded-2xl border ${darkMode ? 'border-slate-700/50' : 'border-gray-200/50'} shadow-lg`}>
                  ...
                  <div className="space-y-3">
                    {recentFiles.map((file, idx) => (
                      <div
                        key={idx}
                        className={`p-4 rounded-xl border cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-[1.02] ${darkMode
